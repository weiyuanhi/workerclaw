...
import { isUiOnlyProduct } from "../product/ui-only.js";
...
function expectDetachedDeliveryMode(delivery: Record<string, unknown>): void {
  expect(delivery.mode).toBe(isUiOnlyProduct() ? "none" : "announce");
}
...
describe("normalizeCronJobCreate", () => {
  ...
  it("defaults isolated agentTurn delivery to announce", () => {
    ...
    expectDetachedDeliveryMode(normalized.delivery as Record<string, unknown>);
  });
