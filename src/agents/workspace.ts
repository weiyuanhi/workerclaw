...
import {
  ...
  WORKSPACE_PLAYBOOKS_README,
} from "../skills/workshop/playbooks.js";
import {
...
async function ensureWorkspacePlaybooksScaffold(workspaceDir: string): Promise<void> {
  const playbooksDir = path.join(workspaceDir, "skills", DEFAULT_PLAYBOOKS_SUBDIR);
  await fs.mkdir(playbooksDir, { recursive: true });
  await writeFileIfMissing(path.join(playbooksDir, "README.md"), WORKSPACE_PLAYBOOKS_README);
}
...
export async function ensureAgentWorkspace(params?: {
  ...
  await ensureWorkspacePlaybooksScaffold(dir);
