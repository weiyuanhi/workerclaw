...
describe("skill_workshop tool", () => {
  ...
  it("creates pending skill proposals without applying them", async () => {
    ...
    const proposalJsonPath = path.join(
      stateDir,
      "skill-workshop",
      "proposals",
      (result.details as { id: string }).id,
    ...
    await expect(
      fs
        .readFile(proposalJsonPath, "utf8")
    ...
    const playbook = await tool.execute("call-playbook", {
      action: "create",
      name: "Fill Purchase Order",
      description: "Vendor portal PO workflow",
      category: "playbook",
      proposal_content: "# Fill Purchase Order\n\n- Open vendor portal\n",
    });
    expect((playbook.details as { targetSkillFile: string }).targetSkillFile).toBe(
