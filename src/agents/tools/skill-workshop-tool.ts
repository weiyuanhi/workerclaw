...
const SKILL_PROPOSAL_CATEGORIES = ["skill", "playbook"] as const;
...
const SkillWorkshopToolSchema = Type.Object(
  {
    ...
    category: Type.Optional(
      stringEnum(SKILL_PROPOSAL_CATEGORIES, {
        description:
          "Proposal target category for action=create. Use playbook for learned multi-step business workflows (stored under workspace skills/playbooks/). Default: skill.",
      }),
    ),
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (action === "create") {
        proposal = await proposeCreateSkill({
          ...
          category: readProposalCategoryParam(params),
          supportFiles,
...
function readProposalCategoryParam(
  params: Record<string, unknown>,
): "skill" | "playbook" | undefined {
  const category = readStringParam(params, "category");
  if (!category) {
    return undefined;
  }
  if (!(SKILL_PROPOSAL_CATEGORIES as readonly string[]).includes(category)) {
    throw new ToolInputError(`category must be one of ${SKILL_PROPOSAL_CATEGORIES.join(", ")}`);
  }
  return category as "skill" | "playbook";
}
