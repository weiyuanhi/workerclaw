...
export function buildSkillWorkshopPromptSection(): string[] {
  return [
    ...
    "For learned multi-step business workflows (for example filling a PO, onboarding a vendor, or repeating a portal checklist), use `action=create` with `category=playbook`. Playbooks land under `skills/playbooks/` after apply.",
    "After completing a multi-step business task, ask whether to capture it as a playbook proposal. Do not apply until the user explicitly approves.",
