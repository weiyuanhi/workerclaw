...
function filterSubCliItems<T>(items: ReadonlyArray<T>, getName: (item: T) => string): ReadonlyArray<T> {
  return filterPrivateQaItems(items, getName);
}

/** Visible sub-CLI descriptors after product gating. */
export const SUB_CLI_DESCRIPTORS = filterSubCliItems(
...
export function getSubCliEntries(): ReadonlyArray<SubCliDescriptor> {
  return filterSubCliItems(subCliCommandCatalog.getDescriptors(), (descriptor) => descriptor.name);
}

/** Return visible sub-CLI names that own child subcommands. */
export function getSubCliCommandsWithSubcommands(): string[] {
  return [...filterSubCliItems(subCliCommandCatalog.getCommandsWithSubcommands(), (command) => command)];
}
...
export function getSubCliParentDefaultHelpCommands(): string[] {
  return [
    ...filterSubCliItems(subCliCommandCatalog.getParentDefaultHelpCommands(), (command) => command),
  ];
}
