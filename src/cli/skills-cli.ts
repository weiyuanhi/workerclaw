...
import type { SkillProposalCategory } from "../skills/workshop/playbooks.js";
import type {
...
function parseSkillProposalCategory(value: string | undefined): SkillProposalCategory | undefined {
  const normalized = value?.trim().toLowerCase();
  if (!normalized) {
    return undefined;
  }
  if (normalized === "playbook" || normalized === "skill") {
    return normalized;
  }
  throw new Error('category must be "skill" or "playbook"');
}
...
export function registerSkillsCli(program: Command) {
  ...
  workshop
    ...
    .option(
      "--category <category>",
      "Proposal target category: skill (default) or playbook (skills/playbooks/)",
    )
    .option("--json", "Output as JSON", false)
    .action(
      async (
        ...
        try {
          ...
          const proposal = await proposeCreateSkill({
            ...
            category: parseSkillProposalCategory(opts.category),
