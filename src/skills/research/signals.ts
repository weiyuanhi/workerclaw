...
import { renderPlaybookProposalBody } from "../workshop/playbooks.js";
...
export function extractDurableInstructionProposal(params: {
  ...
  return {
    ...
    content: renderPlaybookProposalBody({
      title: topic.title,
      summary: `Repeatable business workflow for ${topic.label}.`,
      steps: [instruction, "Verify the result before final reply."],
      pitfalls: ["Avoid copying transcript noise; keep only durable steps."],
      evidence: instruction,
    }),
  };
}
