...
import { readWorkspaceSkillFile } from "../workshop/store.js";
...
export async function runSkillResearchAutoCapture(params: {
  ...
  try {
    ...
    const existingEntry = resolveSkillStatusEntry(status.skills, proposal.skillName);
    const existingSkill = existingEntry
      ? await readWorkspaceSkillFile(existingEntry.filePath)
      : null;
    const result =
      existingSkill === null
        ...
        : await proposeUpdateSkill({
            ...
            config: params.config,
            agentId: params.ctx.agentId,
            skillName: existingEntry?.name ?? proposal.skillName,
            description: proposal.description,
