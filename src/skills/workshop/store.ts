...
export function resolveSkillProposalTarget(params: {
  ...
  const category = params.category === "playbook" ? "playbook" : "skill";
  const playbooksSubdir = params.playbooksSubdir?.trim() || "playbooks";
  const skillDir =
    category === "playbook"
      ? path.resolve(params.workspaceDir, "skills", playbooksSubdir, skillKey)
      : path.resolve(params.workspaceDir, "skills", skillKey);
  ...
  return { skillKey, skillDir, skillFile, category };
}

/** Resolves a create target from config and optional category hint. */
export function resolveCreateSkillProposalTarget(params: {
  workspaceDir: string;
  skillName: string;
  category?: SkillProposalCategory;
  config?: OpenClawConfig;
}) {
  const category = resolveSkillProposalCategory({
  ...
  return resolveSkillProposalTarget({
    ...
    category,
    playbooksSubdir: playbooks.subdir,
  });
}
