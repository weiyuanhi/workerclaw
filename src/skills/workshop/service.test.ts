...
describe("skill workshop proposals", () => {
  ...
  it("creates and applies playbook proposals under skills/playbooks/", async () => {
    const workspaceDir = await makeWorkspace();
    const proposal = await proposeCreateSkill({
      workspaceDir,
      name: "Fill Purchase Order",
      description: "Vendor portal PO submission workflow",
      category: "playbook",
      content:
        "# Fill Purchase Order\n\n## Workflow\n\n- Open vendor portal\n- Submit PO\n",
      createdBy: "skill-workshop",
    ...
    expect(proposal.record.target.category).toBe("playbook");
    expect(proposal.record.target.skillFile).toBe(
