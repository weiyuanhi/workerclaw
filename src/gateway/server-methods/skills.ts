...
function buildPlaybookDraftAgentInstruction() {
  return [
    "Draft a Skill Workshop playbook proposal from the recent conversation in this session.",
    "",
    "Use `skill_workshop` with `action=create` and `category=playbook`.",
    "Infer a concise playbook name and description from the workflow you just helped with.",
    "Include ordered steps, verification checks, and pitfalls in `proposal_content`.",
    "Do not apply, approve, reject, quarantine, or install unless the user explicitly asks.",
    "If the workflow is unclear or too trivial for a playbook, ask one short clarifying question instead.",
  ].join("\n");
}
...
export const skillsHandlers: GatewayRequestHandlers = {
  ...
  "skills.proposals.requestDraftPlaybook": async (opts) => {
    const { params, respond, context } = opts;
    await runSkillsProposalWorkspaceHandler({
      method: "skills.proposals.requestDraftPlaybook",
      rawParams: params,
      respond,
      context,
