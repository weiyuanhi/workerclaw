...
describe("skills proposal gateway handlers", () => {
  ...
  it("creates playbook proposals under skills/playbooks/", async () => {
    const create = await callHandler("skills.proposals.create", {
      name: "Fill Purchase Order",
      description: "Vendor portal PO workflow",
      category: "playbook",
      content: "# Fill Purchase Order\n\n- Open vendor portal\n",
    });
    expect(create.ok).toBe(true);
    const created = create.response as { record: { target: { skillFile: string; category: string } } };
    expect(created.record.target.category).toBe("playbook");
    expect(created.record.target.skillFile).toBe(
      path.join(mocks.workspaceDir, "skills", "playbooks", "fill-purchase-order", "SKILL.md"),
    );
  });
  ...
  it("starts playbook draft chat turns with visible message and server-built context", async () => {
    const result = await callHandler("skills.proposals.requestDraftPlaybook", {
      message: "Draft a playbook from our conversation.",
      sessionKey: "agent:main:session:chat",
      targetAgentId: "draft-target",
      idempotencyKey: "playbook-draft-run-1",
    });
    ...
    expect(String(forwarded.params?.systemProvenanceReceipt)).toContain(
      "Use `skill_workshop` with `action=create` and `category=playbook`.",
    );
    expect(String(forwarded.params?.systemProvenanceReceipt)).not.toContain(
      "Draft a playbook from our conversation.",
    );
  });
