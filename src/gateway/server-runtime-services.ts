...
import {
  ...
  shouldSkipHeartbeatRunner,
  shouldSkipOutboundDeliveryRecovery,
} from "../product/ui-only.js";
...
export function activateGatewayScheduledServices(params: {
  ...
  const heartbeatRunner = shouldSkipHeartbeatRunner()
    ? createNoopHeartbeatRunner()
    : startHeartbeatRunner({ cfg: params.cfgAtStart });
  if (params.startCron !== false && !shouldSkipCronService()) {
    startGatewayCronWithLogging({
      cron: params.cron,
      logCron: params.logCron,
    });
  }
  if (!shouldSkipOutboundDeliveryRecovery()) {
