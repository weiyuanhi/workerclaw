...
export const CONFIG_HINTS_ZH_CN: ConfigHintMaps = {
  ...
  "approvals.exec.enabled": "Forward Exec Approvals",
  "approvals.exec.mode": "Approval Forwarding Mode",
  "approvals.exec.agentFilter": "Approval 代理 Filter",
  "approvals.exec.sessionFilter": "Approval 会话 Filter",
  "approvals.exec.targets": "Approval Forwarding Targets",
  "approvals.exec.targets[].channel": "Approval Target 频道",
  "approvals.exec.targets[].to": "Approval Target Destination",
  "approvals.exec.targets[].accountId": "Approval Target Account ID",
  "approvals.exec.targets[].threadId": "Approval Target Thread ID",
  "approvals.plugin": "Plugin Approval Forwarding",
  "approvals.plugin.enabled": "Forward Plugin Approvals",
  "approvals.plugin.mode": "Plugin Approval Forwarding Mode",
  "approvals.plugin.agentFilter": "Plugin Approval 代理 Filter",
  "approvals.plugin.sessionFilter": "Plugin Approval 会话 Filter",
  "approvals.plugin.targets": "Plugin Approval Forwarding Targets",
  "approvals.plugin.targets[].channel": "Plugin Approval Target 频道",
  "approvals.plugin.targets[].to": "Plugin Approval Target Destination",
  "approvals.plugin.targets[].accountId": "Plugin Approval Target Account ID",
  "approvals.plugin.targets[].threadId": "Plugin Approval Target Thread ID",
  "tools.message.allowCrossContextSend": "Allow Cross-Context Messaging",
  ...
  "gateway.nodes.allowCommands": "Gateway Node 允许列表 (Extra 命令)",
  ...
  "commands.nativeSkills": "Native Skill 命令",
  ...
  "cron.retry.backoffMs": "Cron Retry Backoff（毫秒）",
  "cron.retry.retryOn": "Cron Retry Error Types",
  "cron.webhook": "Cron Legacy Webhook (Deprecated)",
  "cron.webhookToken": "Cron Webhook Bearer Token",
  "cron.sessionRetention": "Cron 会话 Retention",
  "cron.runLog": "Cron Run Log Pruning",
  "cron.runLog.maxBytes": "Cron Run Log 最大 字节",
  "cron.runLog.keepLines": "Cron Run Log 保持 Lines",
  "hooks": "Hooks",
  "hooks.enabled": "Hooks 已启用",
  "hooks.path": "Hooks Endpoint Path",
  "hooks.token": "Hooks Auth Token",
  "hooks.defaultSessionKey": "Hooks 默认 会话 Key",
  "hooks.allowRequestSessionKey": "Hooks Allow Request 会话 Key",
  "hooks.allowedSessionKeyPrefixes": "Hooks Allowed 会话 Key Prefixes",
  "hooks.allowedAgentIds": "Hooks Allowed 代理 IDs",
  "hooks.maxBodyBytes": "Hooks 最大 Body 字节",
  "hooks.presets": "Hooks Presets",
  "hooks.transformsDir": "Hooks Transforms Directory",
  "hooks.mappings": "Hook Mappings",
  "hooks.mappings[].id": "Hook Mapping ID",
  ...
  "hooks.mappings[].wakeMode": "Hook Mapping Wake Mode",
  "hooks.mappings[].name": "Hook Mapping Name",
  "hooks.mappings[].agentId": "Hook Mapping 代理 ID",
  "hooks.mappings[].sessionKey": "Hook Mapping 会话 Key",
  "hooks.mappings[].messageTemplate": "Hook Mapping Message Template",
  "hooks.mappings[].textTemplate": "Hook Mapping Text Template",
  "hooks.mappings[].deliver": "Hook Mapping Deliver Reply",
  "hooks.mappings[].allowUnsafeExternalContent": "Hook Mapping Allow Unsafe External Content",
  "hooks.mappings[].channel": "Hook Mapping Delivery 频道",
  "hooks.mappings[].to": "Hook Mapping Delivery Destination",
  "hooks.mappings[].model": "Hook Mapping Model 覆盖",
  "hooks.mappings[].thinking": "Hook Mapping Thinking 覆盖",
  "hooks.mappings[].timeoutSeconds": "Hook Mapping 超时（秒）",
  "hooks.mappings[].transform": "Hook Mapping Transform",
  "hooks.mappings[].transform.module": "Hook Transform Module",
  "hooks.mappings[].transform.export": "Hook Transform Export",
  "hooks.gmail": "Gmail Hook",
  ...
  "hooks.gmail.hookUrl": "Gmail Hook Callback URL",
  "hooks.gmail.includeBody": "Gmail Hook Include Body",
  "hooks.gmail.maxBytes": "Gmail Hook 最大 Body 字节",
  "hooks.gmail.renewEveryMinutes": "Gmail Hook Renew 间隔 (min)",
  "hooks.gmail.allowUnsafeExternalContent": "Gmail Hook Allow Unsafe External Content",
  "hooks.gmail.serve": "Gmail Hook 本地 Server",
  "hooks.gmail.serve.bind": "Gmail Hook Server Bind Address",
  "hooks.gmail.serve.port": "Gmail Hook Server Port",
  "hooks.gmail.serve.path": "Gmail Hook Server Path",
  "hooks.gmail.tailscale": "Gmail Hook Tailscale",
  ...
  "hooks.gmail.tailscale.path": "Gmail Hook Tailscale Path",
  "hooks.gmail.tailscale.target": "Gmail Hook Tailscale Target",
  "hooks.gmail.model": "Gmail Hook Model 覆盖",
  "hooks.gmail.thinking": "Gmail Hook Thinking 覆盖",
  "hooks.internal": "Internal Hooks",
  "hooks.internal.enabled": "Internal Hooks 已启用",
  "hooks.internal.entries": "Internal Hook Entries",
  "hooks.internal.load": "Internal Hook Loader",
  "hooks.internal.load.extraDirs": "Internal Hook Extra Directories",
  "hooks.internal.installs": "Internal Hook Install Records",
  ...
  "bindings": "Top-level binding rules 用于 routing 与 persistent ACP conversation ownership. 使用 type=route 用于 normal routing 与 type=acp 用于 persistent ACP harness bindings.",
  ...
  "approvals": "Approval routing controls 用于 forwarding exec 与 plugin approval requests 到 chat destinations outside the originating session. 保持 these disabled unless operators need explicit out-of-band approval visibility.",
  "approvals.exec": "Groups exec-approval forwarding behavior including enablement, routing mode, filters, 与 explicit targets. Configure here 当 approval prompts must reach operational channels instead of only the origin thread.",
  "approvals.exec.enabled": "启用s forwarding of exec approval requests 到 configured delivery destinations (default: false). 保持 disabled in low-risk setups 与 enable only 当 human approval responders need channel-visible prompts.",
  "approvals.exec.mode": "控制 where approval prompts are sent: \"session\" uses origin chat, \"targets\" uses configured targets, 与 \"both\" sends 到 both paths. 使用 \"session\" as baseline 与 expand only 当 operational workflow requires redundancy.",
  "approvals.exec.agentFilter": "可选 allowlist of agent IDs eligible 用于 forwarded approvals, 用于 example `[\"primary\", \"ops-agent\"]`. 使用 this 到 limit forwarding blast radius 与 avoid notifying channels 用于 unrelated agents.",
  "approvals.exec.sessionFilter": "可选 session-key filters matched as substring 或 regex-style patterns, 用于 example `[\"discord:\", \"^agent:ops:\"]`. 使用 narrow patterns so only intended approval contexts are forwarded 到 shared destinations.",
  "approvals.exec.targets": "Explicit delivery targets used 当 forwarding mode includes targets, each 使用 channel 与 destination details. 保持 target lists least-privilege 与 validate each destination 在…之前 enabling broad forwarding.",
  "approvals.exec.targets[].channel": "频道/provider ID used 用于 forwarded approval delivery, such as discord, slack, 或 a plugin channel id. 使用 valid channel IDs only so approvals do not silently fail due 到 unknown routes.",
  "approvals.exec.targets[].to": "Destination identifier inside the target channel (channel ID, user ID, 或 thread root depending on provider). Verify semantics 每 provider because destination format differs across channel integrations.",
  "approvals.exec.targets[].accountId": "可选 account selector 用于 multi-account channel setups 当 approvals must route through a specific account context. 使用 this only 当 the target channel has multiple configured identities.",
  "approvals.exec.targets[].threadId": "可选 thread/topic target 用于 channels that support threaded delivery of forwarded approvals. 使用 this 到 keep approval traffic contained in operational threads instead of main channels.",
  "approvals.plugin": "Groups plugin-approval forwarding behavior including enablement, routing mode, filters, 与 explicit targets. Independent of exec approval forwarding. Configure here 当 plugin approval prompts must reach operational channels.",
  "approvals.plugin.enabled": "启用s forwarding of plugin approval requests 到 configured delivery destinations (default: false). Independent of approvals.exec.enabled.",
  "approvals.plugin.mode": "控制 where plugin approval prompts are sent: \"session\" uses origin chat, \"targets\" uses configured targets, 与 \"both\" sends 到 both paths.",
  "approvals.plugin.agentFilter": "可选 allowlist of agent IDs eligible 用于 forwarded plugin approvals, 用于 example `[\"primary\", \"ops-agent\"]`. 使用 this 到 limit forwarding blast radius.",
  "approvals.plugin.sessionFilter": "可选 session-key filters matched as substring 或 regex-style patterns, 用于 example `[\"discord:\", \"^agent:ops:\"]`. 使用 narrow patterns so only intended approval contexts are forwarded.",
  "approvals.plugin.targets": "Explicit delivery targets used 当 plugin approval forwarding mode includes targets, each 使用 channel 与 destination details.",
  "approvals.plugin.targets[].channel": "频道/provider ID used 用于 forwarded plugin approval delivery, such as discord, slack, 或 a plugin channel id.",
  "approvals.plugin.targets[].to": "Destination identifier inside the target channel (channel ID, user ID, 或 thread root depending on provider).",
  "approvals.plugin.targets[].accountId": "可选 account selector 用于 multi-account channel setups 当 plugin approvals must route through a specific account context.",
  "approvals.plugin.targets[].threadId": "可选 thread/topic target 用于 channels that support threaded delivery of forwarded plugin approvals.",
  ...
  "commands": "控制聊天命令界面、所有者门控以及跨提供商的提升命令访问行为。除非需要更严格的操作员控制或更广的命令可用性，否则保持默认即可。",
  "commands.native": "在支持命令注册的频道（Discord、Slack、Telegram）中注册原生斜杠/菜单命令。除非有意仅使用纯文本命令工作流，否则保持启用以便发现命令。",
  "commands.nativeSkills": "注册原生技能命令，使用户可在支持的提供商命令菜单中直接调用技能。请与技能策略保持一致，使暴露的命令符合操作员预期。",
  "commands.text": "在可用时除原生命令界面外，还启用聊天输入中的文本命令解析。对于不支持原生命令注册的频道，请保持启用以兼容。",
  "commands.bash": "允许 bash 聊天命令（`!`；`/bash` 别名）运行主机 Shell 命令（默认：false；需要 tools.elevated）。",
  "commands.bashForegroundMs": "bash 在转入后台前等待的时间（默认：2000；0 表示立即后台运行）。",
  "commands.config": "允许 /config 聊天命令读写磁盘上的配置（默认：false）。",
  "commands.mcp": "允许 /mcp 聊天命令管理 mcp.servers 下的 OpenClaw MCP 服务器配置（默认：false）。",
  "commands.plugins": "允许 /plugins 聊天命令列出已发现插件并切换配置中的插件启用状态（默认：false）。",
  "commands.debug": "允许 /debug 聊天命令进行仅运行时覆盖（默认：false）。",
  "commands.restart": "允许 /restart 与 Gateway 重启工具操作（默认：true）。",
  "commands.useAccessGroups": "对命令强制执行访问组允许列表/策略。",
  "commands.ownerAllowFrom": "所有者范围命令的显式所有者允许列表。请使用频道原生 ID（可选前缀，如 \"whatsapp:+15551234567\"）。\"*\" 会被忽略。",
  "commands.ownerDisplay": "控制系统提示中所有者 ID 的渲染方式。允许值：raw、hash。默认：raw。",
  "commands.ownerDisplaySecret": "当 ownerDisplay=hash 时用于 HMAC 哈希所有者 ID 的可选密钥。建议使用环境变量替换。",
  "commands.allowFrom": "按频道与发送者为所有者级命令界面定义提升命令允许规则。请使用较窄的提供商特定身份，避免将特权命令暴露给广泛聊天受众。",
  ...
  "cron": "Global scheduler settings 用于 stored cron jobs, run concurrency, delivery fallback, 与 run-session retention. 保持 defaults unless you are scaling job volume 或 integrating external webhook receivers.",
  "cron.enabled": "启用s cron job execution 用于 stored schedules managed by the gateway. 保持 enabled 用于 normal reminder/automation flows, 与 disable only 到 pause all cron execution without deleting jobs.",
  "cron.store": "Path 到 the cron job store file used 到 persist scheduled jobs across restarts. Set an explicit path only 当 you need custom storage layout, backups, 或 mounted volumes.",
  "cron.maxConcurrentRuns": "默认s 到 8. Limits how many cron jobs can execute at the same time 当 multiple schedules fire together, including isolated agent-turn LLM execution on the dedicated cron-nested lane. 使用 lower values 到 protect CPU/memory under heavy automation load, 或 raise carefully 用于 higher throughput.",
  "cron.retry": "覆盖s the default retry policy 用于 one-shot jobs 当 they fail 使用 transient errors (rate limit, overloaded, network, server_error). Omit 到 use defaults: maxAttempts 3, backoffMs [30000, 60000, 300000], retry all transient types.",
  "cron.retry.maxAttempts": "最大 retries 用于 one-shot jobs on transient errors 在…之前 permanent disable (default: 3).",
  "cron.retry.backoffMs": "Backoff delays in ms 用于 each retry attempt (default: [30000, 60000, 300000]). 使用 shorter values 用于 faster retries.",
  "cron.retry.retryOn": "Error types 到 retry: rate_limit, overloaded, network, timeout, server_error. 使用 到 restrict which errors trigger retries; omit 到 retry all transient types.",
  "cron.webhook": "Deprecated legacy fallback webhook URL used by `openclaw doctor --fix` 到 migrate old jobs 使用 `notify=true`. 运行时 delivery uses per-job `delivery.mode=\"webhook\"` plus `delivery.to`, 或 `delivery.completionDestination` 当 preserving announce delivery.",
  "cron.webhookToken": "Bearer token attached 到 cron webhook POST deliveries 当 webhook mode is used. Prefer secret/env substitution 与 rotate this token regularly if shared webhook endpoints are internet-reachable.",
  "cron.sessionRetention": "控制 how long completed cron run sessions are kept 在…之前 pruning (`24h`, `7d`, `1h30m`, 或 `false` 到 disable pruning; default: `24h`). 使用 shorter retention 到 reduce storage growth on high-frequency schedules.",
  "cron.runLog": "Pruning controls 用于 per-job cron run history. Run history is stored in SQLite; max字节 remains accepted 用于 older file-backed run logs.",
  "cron.runLog.maxBytes": "Compatibility setting 用于 older file-backed cron run logs (for example `2mb`, default `2000000`). SQLite run history pruning is row-count based.",
  "cron.runLog.keepLines": "How many trailing run-history rows 到 retain 每 cron job (default `2000`). Increase 用于 longer forensic history 或 lower 用于 smaller disks.",
  "hooks": "Inbound webhook automation surface 用于 mapping external events into wake 或 agent actions in OpenClaw. 保持 this locked down 使用 explicit token/session/agent controls 在…之前 exposing it beyond trusted networks.",
  "hooks.enabled": "启用s the hooks endpoint 与 mapping execution pipeline 用于 inbound webhook requests. 保持 disabled unless you are actively routing external events into the gateway.",
  "hooks.path": "HTTP path used by the hooks endpoint (for example `/hooks`) on the gateway control server. 使用 a non-guessable path 与 combine it 使用 token validation 用于 defense in depth.",
  "hooks.token": "Shared bearer token checked by hooks ingress 用于 request authentication 在…之前 mappings run. Treat holders as full-trust callers 用于 the hook ingress surface, not as a separate non-owner role. 使用 environment substitution 与 rotate regularly 当 webhook endpoints are internet-accessible.",
  "hooks.defaultSessionKey": "Fallback session key used 用于 hook deliveries 当 a request does not provide one through allowed channels. 使用 a stable but scoped key 到 avoid mixing unrelated automation conversations.",
  "hooks.allowRequestSessionKey": "Allows callers 到 supply a session key in hook requests 当 true, enabling caller-controlled routing. 保持 false unless trusted integrators explicitly need custom session threading.",
  "hooks.allowedSessionKeyPrefixes": "允许列表 of accepted session-key prefixes 用于 inbound hook requests 当 caller-provided keys are enabled. 使用 narrow prefixes 到 prevent arbitrary session-key injection.",
  "hooks.allowedAgentIds": "允许列表 of effective agent IDs that hook requests 与 mappings are allowed 到 target, including default-agent routing 当 agentId is omitted. 使用 this 到 constrain automation events 到 dedicated service agents 与 reduce blast radius if a hook token is exposed.",
  "hooks.maxBodyBytes": "最大 accepted webhook payload size in bytes 在…之前 the request is rejected. 保持 this bounded 到 reduce abuse risk 与 protect memory usage under bursty integrations.",
  "hooks.presets": "Named hook preset bundles applied at load time 到 seed standard mappings 与 behavior defaults. 保持 preset usage explicit so operators can audit which automations are active.",
  "hooks.transformsDir": "Base directory 用于 hook transform modules referenced by mapping transform.module paths. 使用 a controlled repo directory so dynamic imports remain reviewable 与 predictable.",
  "hooks.mappings": "Ordered mapping rules that match inbound hook requests 与 choose wake 或 agent actions 使用 optional delivery routing. 使用 specific mappings first 到 avoid broad pattern rules capturing everything.",
  "hooks.mappings[].id": "可选 stable identifier 用于 a hook mapping entry used 用于 auditing, troubleshooting, 与 targeted updates. 使用 unique IDs so logs 与 config diffs can reference mappings unambiguously.",
  "hooks.mappings[].match": "Grouping object 用于 mapping match predicates such as path 与 source 在…之前 action routing is applied. 保持 match criteria specific so unrelated webhook traffic does not trigger automations.",
  "hooks.mappings[].match.path": "Path match condition 用于 a hook mapping, usually compared against the inbound request path. 使用 this 到 split automation behavior by webhook endpoint path families.",
  "hooks.mappings[].match.source": "Source match condition 用于 a hook mapping, typically set by trusted upstream metadata 或 adapter logic. 使用 stable source identifiers so routing remains deterministic across retries.",
  "hooks.mappings[].action": "Mapping action type: \"wake\" triggers agent wake flow, while \"agent\" sends directly 到 agent handling. 使用 \"agent\" 用于 immediate execution 与 \"wake\" 当 heartbeat-driven processing is preferred.",
  "hooks.mappings[].wakeMode": "Wake scheduling mode: \"now\" wakes immediately, while \"next-heartbeat\" defers until the next heartbeat cycle. 使用 deferred mode 用于 lower-priority automations that can tolerate slight delay.",
  "hooks.mappings[].name": "Human-readable mapping display name used in diagnostics 与 operator-facing config UIs. 保持 names concise 与 descriptive so routing intent is obvious 在…期间 incident review.",
  "hooks.mappings[].agentId": "Target agent ID 用于 mapping execution 当 action routing should not use defaults. 使用 dedicated automation agents 到 isolate webhook behavior 来自 interactive operator sessions.",
  "hooks.mappings[].sessionKey": "Explicit session key override 用于 mapping-delivered messages 到 control thread continuity. 使用 stable scoped keys so repeated events correlate without leaking into unrelated conversations.",
  "hooks.mappings[].messageTemplate": "Template 用于 synthesizing structured mapping input into the final message content sent 到 the target action path. 保持 templates deterministic so downstream parsing 与 behavior remain stable.",
  "hooks.mappings[].textTemplate": "Text-only fallback template used 当 rich payload rendering is not desired 或 not supported. 使用 this 到 provide a concise, consistent summary string 用于 chat delivery surfaces.",
  "hooks.mappings[].deliver": "控制 whether mapping execution results are delivered back 到 a channel destination versus being processed silently. 禁用 delivery 用于 background automations that should not post user-facing output.",
  "hooks.mappings[].allowUnsafeExternalContent": "When true, mapping content may include less-sanitized external payload data in generated messages. 保持 false by default 与 enable only 用于 trusted sources 使用 reviewed transform logic.",
  "hooks.mappings[].channel": "Delivery channel override 用于 mapping outputs (for example \"last\", \"telegram\", \"discord\", \"slack\", \"signal\", \"imessage\", 或 \"msteams\"). 保持 channel overrides explicit 到 avoid accidental cross-channel sends.",
  "hooks.mappings[].to": "Destination identifier inside the selected channel 当 mapping replies should route 到 a fixed target. Verify provider-specific destination formats 在…之前 enabling production mappings.",
  "hooks.mappings[].model": "可选 model override 用于 mapping-triggered runs 当 automation should use a different model than agent defaults. 使用 this sparingly so behavior remains predictable across mapping executions.",
  "hooks.mappings[].thinking": "可选 thinking-effort override 用于 mapping-triggered runs 到 tune latency versus reasoning depth. 保持 low 或 minimal 用于 high-volume hooks unless deeper reasoning is clearly required.",
  "hooks.mappings[].timeoutSeconds": "最大 runtime allowed 用于 mapping action execution 在…之前 timeout handling applies. 使用 tighter limits 用于 high-volume webhook sources 到 prevent queue pileups.",
  "hooks.mappings[].transform": "Transform configuration block defining module/export preprocessing 在…之前 mapping action handling. 使用 transforms only 来自 reviewed code paths 与 keep behavior deterministic 用于 repeatable automation.",
  "hooks.mappings[].transform.module": "Relative transform module path loaded 来自 hooks.transformsDir 到 rewrite incoming payloads 在…之前 delivery. 保持 modules local, reviewed, 与 free of path traversal patterns.",
  "hooks.mappings[].transform.export": "Named export 到 invoke 来自 the transform module; defaults 到 module default export 当 omitted. Set this 当 one file hosts multiple transform handlers.",
  "hooks.gmail": "Gmail push integration settings used 用于 Pub/Sub notifications 与 optional local callback serving. 保持 this scoped 到 dedicated Gmail automation accounts where possible.",
  "hooks.gmail.account": "Google account identifier used 用于 Gmail watch/subscription operations in this hook integration. 使用 a dedicated automation mailbox account 到 isolate operational permissions.",
  "hooks.gmail.label": "可选 Gmail label filter limiting which labeled messages trigger hook events. 保持 filters narrow 到 avoid flooding automations 使用 unrelated inbox traffic.",
  "hooks.gmail.topic": "Google Pub/Sub topic name used by Gmail watch 到 publish change notifications 用于 this account. Ensure the topic IAM grants Gmail publish access 在…之前 enabling watches.",
  "hooks.gmail.subscription": "Pub/Sub subscription consumed by the gateway 到 receive Gmail change notifications 来自 the configured topic. 保持 subscription ownership clear so multiple consumers do not race unexpectedly.",
  "hooks.gmail.hookUrl": "Public callback URL Gmail 或 intermediaries invoke 到 deliver notifications into this hook pipeline. 保持 this URL protected 使用 token validation 与 restricted network exposure.",
  "hooks.gmail.includeBody": "When true, fetch 与 include email body content 用于 downstream mapping/agent processing. 保持 false unless body text is required, because this increases payload size 与 sensitivity.",
  "hooks.gmail.allowUnsafeExternalContent": "Allows less-sanitized external Gmail content 到 pass into processing 当 enabled. 保持 disabled 用于 safer defaults, 与 enable only 用于 trusted mail streams 使用 controlled transforms.",
  "hooks.gmail.serve": "本地 callback server settings block 用于 directly receiving Gmail notifications without a separate ingress layer. 启用 only 当 this process should terminate webhook traffic itself.",
  "hooks.gmail.pushToken": "Shared secret token required on Gmail push hook callbacks 在…之前 processing notifications. 使用 env substitution 与 rotate if callback endpoints are exposed externally.",
  "hooks.gmail.maxBytes": "最大 Gmail payload bytes processed 每 event 当 includeBody is enabled. 保持 conservative limits 到 reduce oversized message processing cost 与 risk.",
  "hooks.gmail.renewEveryMinutes": "Renewal cadence in minutes 用于 Gmail watch subscriptions 到 prevent expiration. Set below provider expiration windows 与 monitor renew failures in logs.",
  "hooks.gmail.serve.bind": "Bind address 用于 the local Gmail callback HTTP server used 当 serving hooks directly. 保持 loopback-only unless external ingress is intentionally required.",
  "hooks.gmail.serve.port": "Port 用于 the local Gmail callback HTTP server 当 serve mode is enabled. 使用 a dedicated port 到 avoid collisions 使用 gateway/control interfaces.",
  "hooks.gmail.serve.path": "HTTP path on the local Gmail callback server where push notifications are accepted. 保持 this consistent 使用 subscription configuration 到 avoid dropped events.",
  "hooks.gmail.tailscale.mode": "Tailscale exposure mode 用于 Gmail callbacks: \"off\", \"serve\", 或 \"funnel\". 使用 \"serve\" 用于 private tailnet delivery 与 \"funnel\" only 当 public internet ingress is required.",
  "hooks.gmail.tailscale": "Tailscale exposure configuration block 用于 publishing Gmail callbacks through Serve/Funnel routes. 使用 private tailnet modes 在…之前 enabling any public ingress path.",
  "hooks.gmail.tailscale.path": "Path published by Tailscale Serve/Funnel 用于 Gmail callback forwarding 当 enabled. 保持 it aligned 使用 Gmail webhook config so requests reach the expected handler.",
  "hooks.gmail.tailscale.target": "本地 service target forwarded by Tailscale Serve/Funnel (for example http://127.0.0.1:8787). 使用 explicit loopback targets 到 avoid ambiguous routing.",
  "hooks.gmail.model": "可选 model override 用于 Gmail-triggered runs 当 mailbox automations should use dedicated model behavior. 保持 unset 到 inherit agent defaults unless mailbox tasks need specialization.",
  "hooks.gmail.thinking": "Thinking effort override 用于 Gmail-driven agent runs: \"off\", \"minimal\", \"low\", \"medium\", 或 \"high\". 保持 modest defaults 用于 routine inbox automations 到 control cost 与 latency.",
  "hooks.internal": "Internal hook runtime settings 用于 bundled/custom event handlers loaded 来自 module paths. 使用 this 用于 trusted in-process automations 与 keep handler loading tightly scoped.",
  "hooks.internal.enabled": "启用s processing 用于 internal hooks 与 configured entries in the internal hook runtime. 保持 disabled unless internal hooks are intentionally configured.",
  "hooks.internal.entries": "Configured internal hook entry records used 到 register concrete runtime handlers 与 metadata. 保持 entries explicit 与 versioned so production behavior is auditable.",
  "hooks.internal.load": "Internal hook loader settings controlling where handler modules are discovered at startup. 使用 constrained load roots 到 reduce accidental module conflicts 或 shadowing.",
  "hooks.internal.load.extraDirs": "Additional directories searched 用于 internal hook modules beyond default load paths. 保持 this minimal 与 controlled 到 reduce accidental module shadowing.",
  "hooks.internal.installs": "Install metadata 用于 internal hook modules, including source 与 resolved artifacts 用于 repeatable deployments. 使用 this as operational provenance 与 avoid manual drift edits.",
