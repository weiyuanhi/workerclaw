...
## Playbooks (learned business workflows)

Use playbooks for multi-step business procedures learned from experience (for
example filling a PO, walking through a vendor portal, or repeating an internal
checklist). They are workspace skills stored under `skills/playbooks/` after
apply.

Create from chat:

```text
Capture the PO fill-in flow we just completed as a playbook named fill-purchase-order.
```

The agent should call `skill_workshop` with `action=create` and
`category=playbook`. Ask the user before applying the proposal.
...
openclaw skills workshop propose-create \
  ...
  --description "Fill a purchase order in the vendor portal" \
  --category playbook \
  --proposal ./PROPOSAL.md
```

Playbook proposals use the same pending → apply lifecycle as other skills.
Updates to live playbooks use `action=update` on the existing skill name.

Configure the target folder with `skills.workshop.playbooks` (default subdir:
`playbooks`). See [Skills config](/tools/skills-config).
